...
data %>%
  ...
  tally_and_perc(Learning_Activities, na.rm = TRUE) %>% 
  filter(!is.na(Learning_Activities)) %>% 
  arrange(desc(n)) %>% 
  rename(`Continuous Learning Post-Workshop` = Learning_Activities)

print(Learning_Activities, digits = getOption("digits"), quote = FALSE,
      na.print = "", zero.print = "0", justify = "none")
...
kable(Learning_Activities, format = "markdown", row.names = FALSE, caption = "Continuous Learning Post-Workshop")
```
...
data %>%
  ...
  gather(col, Carpentry_Involvement) %>% 
  group_by(Carpentry_Involvement) %>% 
  tally_and_perc(Carpentry_Involvement, na.rm = TRUE) %>%  
  filter(!is.na(Carpentry_Involvement)) %>% 
  arrange(desc(n)) %>% 
  rename(`Carpentry Involvement Post-Workshop` = Carpentry_Involvement)

print(Carpentry_Involvement, digits = getOption("digits"), quote = FALSE,
      na.print = "", zero.print = "0", justify = "none")
```
```{r}
kable(Carpentry_Involvement, format = "markdown", row.names = FALSE, caption = "Respondents Self-Reported Inolvement in the Carpentries Post-Workshop")
```
